...
class Ad
{
     ...
     * @Assert\Length(min=10,max=255,minMessage="Le titre doit faire plus de 10 caracteres !",maxMessage="Le titre ne doit pas depasser 255 caracteres")
     */
    private $title;
    ...
    public function initializeSlug(){
        if (empty($this->slug))
        {
            ...
            //just me testing itd
            $slugify= new Slugify();
            $this->slug=$slugify->slugify($this->title)."-".$slugPlus;
        }
    }
    ...
    public function getNotAvailableDays(){
        ...
        foreach ($this->bookings as $booking){
             //Calculer les jours qui se trouvent entre la date déarrivé et la date de départ
            $resultat= range($booking->getStartDate()->getTimestamp(),
            $booking->getEndDate()->getTimestamp(),
            24*60*60
            );

            // on a un tableau des TimeStamp on doit les transformer en DateTime
